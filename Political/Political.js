...
import './Political.css'; // Import the loader CSS
...
const News = () => {
  ...
  const apiKey = '363ba0a406994160ab8015036963541c'; // Replace with your News API key
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchNews = async () => {
      try {
        const response = await axios.get(`https://newsapi.org/v2/everything?q=Andhra+Pradesh+political&apiKey=${apiKey}`);
